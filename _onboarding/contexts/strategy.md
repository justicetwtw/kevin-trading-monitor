...
> ✅ **狀態更新（2026-07）**
>
> `docs/strategy_v4.md` 已由 Kevin 補入**完整策略總架構 v5 全文**，並在 `AGENTS.md` §4 指定為策略 single source of truth。
> **AI 不得自行增删、改寫或推導策略語意**；若 code 與該檔衝突，必須回報並用 PR 釐清，不得靜默選一邊。
...
| 來源 | 內容 | 權威程度 |
|---|---|---|
| `docs/strategy_v4.md` | **完整 v5 全文**（投資哲學 / 帳戶配置 / 三大訊號 / Layer 0,0+,F / 出場規則 / 學習鎖 / 季節性 / EV）| ✅ 策略 single source of truth（AGENTS.md §4） |
...
**結論**：策略「完整論述全文」在 `docs/strategy_v4.md`（v5），「執行細節」在程式碼；兩者衝突時回報並以 PR 釐清（AGENTS.md §4 紅線 1）。
...
- `docs/strategy_v4.md` 全文維護 = **使用者的工作**，不是 AI 的。AI 可在使用者明確提供內容時協助貼入 / 排版，但不得無中生有或自行改寫語意。
- 若使用者問「策略是什麼」，回答應指向 `docs/strategy_v4.md`（v5 全文）與上述可查證來源，**不要從程式碼反推出一套你以為的策略論述覆蓋該檔**。
